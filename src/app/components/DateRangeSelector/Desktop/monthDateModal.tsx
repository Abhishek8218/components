...
const MonthModal = ({
  ...
  <div className={`absolute bottom-0 min-h-fit top-8 inset-0 flex items-center justify-center max-w-[272px] max-h-[365px]  ml-4 mt-12 rounded shadow ${showMonthModal ? '' : 'hidden'}`}>
      ...
      <div className="grid grid-cols-1 gap-2 overflow-y-scroll max-h-[250px] custom-scrollbar ">
        {MONTH_NAMES.map((monthName, index) => (
          <p
            ...
            className={`cursor-pointer px-2 py-1 rounded-lg transition text-sm text-center  max-w-[200px] ${index === month ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-300'}`}
          >
            {monthName}
          </p>
        ))}
