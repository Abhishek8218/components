...
var Tour = dynamic(function () { return import('reactour'); }, {
    ssr: false,
});
var steps = [
    {
...
var Page = function () {
    var _a;
    var _b = useState(false), isTourOpen = _b[0], setIsTourOpen = _b[1];
    var _c = useState(false), nextStep = _c[0], setNextStep = _c[1];
    ...
    useEffect(function () {
        ...
        return function () { return clearTimeout(timer); }; // Clean up the timer if the component unmounts
    }, []);
    if (!isTourOpen) {
    ...
    var handleStepChange = function (stepIndex) {
        console.log("Step ".concat(stepIndex, " changed"));
        // Stop the previous audio
        if (audioRef.current) {
            audioRef.current.pause();
            audioRef.current.currentTime = 0;
            console.log("Audio for step ".concat(stepIndex, " has been stopped"));
        }
        // Play the new audio
        var audio = new Audio(steps[stepIndex].audio);
        audioRef.current = audio;
        // Store the reference to the current audio
        audio.play();
        audio.addEventListener('ended', function () {
            console.log("Audio for step ".concat(stepIndex, " has finished playing"));
    ...
    <Modal isOpen={isModalOpen} onClose={function () { return setIsModalOpen(false); }} onStartTour={startTour}/>
      ...
      <main className="bg-gray-100 bg-opacity-25">
        ...
        {typeof window !== 'undefined' && (<Tour steps={steps} isOpen={isTourOpen} getCurrentStep={function (stepIndex) { handleStepChange(stepIndex); }} onRequestClose={function () { return setIsTourOpen(false); }} goToStep={currentStep} className='bg-red-500'/>)}
    </div>);
};
export default Page;
