...
var MobileTimePickerModal = function (_a) {
    ...
    var scrollToSelected = function () {
        if (hourRef.current && minuteRef.current) {
            var initialHourScroll = middleHourIndex * ITEM_HEIGHT + selectedHour * ITEM_HEIGHT;
            var initialMinuteScroll = middleMinuteIndex * ITEM_HEIGHT + selectedMinute * ITEM_HEIGHT;
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex space-x-4 w-full">
            <div className="w-1/2">
              ...
              <div className="overflow-y-scroll h-48 no-scrollbar" ref={hourRef} onScroll={handleHourScroll}>
                <div className="flex flex-col items-center pt-[50%]">
                  {extendedHours.map(function (hour, index) { return (<div key={index} className={"cursor-pointer text-center py-2 transition duration-150 ease-in-out ".concat(selectedHour === hour ? 'text-blue-500 text-xl font-bold' : 'text-gray-700 text-lg')} style={{ height: ITEM_HEIGHT }}>
                      {('0' + hour).slice(-2)}
                    </div>); })}
                </div>
              </div>
            ...
            <div className="w-1/2">
              ...
              <div className="overflow-y-scroll h-48 no-scrollbar" ref={minuteRef} onScroll={handleMinuteScroll}>
                <div className="flex flex-col items-center pt-[50%]">
                  {extendedMinutes.map(function (minute, index) { return (<div key={index} className={"cursor-pointer text-center py-2 transition duration-150 ease-in-out ".concat(selectedMinute === minute ? 'text-blue-500 text-xl font-bold' : 'text-gray-700 text-lg')} style={{ height: ITEM_HEIGHT }}>
                      {('0' + minute).slice(-2)}
                    </div>); })}
                </div>
              </div>
