...
var YearModal = function (_a) {
    ...
    <button className='text-blue-500 text-sm  ' onClick={setCurrentYear}>Current</button>
      ...
      <div className="grid grid-cols-3 overflow-y-scroll custom-scrollbar max-h-[250px]">
        {Array.from({ length: 61 }, function (_, i) { return new Date().getFullYear() - 60 + i; }).map(function (yearOption) { return (<p key={yearOption} data-year={yearOption} onClick={function () { return handleYearSelect(yearOption); }} className={"m-1 px-2 py-1 rounded-lg transition text-sm duration-200 text-center hover:cursor-pointer ".concat(yearOption === year ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300')}>
            {yearOption}
          </p>); })}
      </div>
    </div>
